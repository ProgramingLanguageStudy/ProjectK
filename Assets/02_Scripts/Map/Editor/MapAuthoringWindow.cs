...
/// <summary>
/// 맵 크기·프리뷰 그리드·홈/장애물/스포너 칸 편집 후 MapData + EnemySpawnerData 생성.
/// Tools / KSS / Map Authoring
/// </summary>
public sealed class MapAuthoringWindow : EditorWindow
{
    const string MenuPath = "Tools/KSS/Map Authoring";
    const string PreviewRootName = "KSS_MapPreviewRoot";
    const string MapsFolder = "Assets/00_Data/Maps";
    ...
    static void Open()
    {
        var w = GetWindow<MapAuthoringWindow>("Map Authoring");
    ...
    void LoadFromMapData(MapData data)
    {
        if (data == null)
            return;
    ...
    void OnGUI()
    {
        ...
        EditorGUILayout.LabelField("기존 맵 불러오기", EditorStyles.boldLabel);
        _mapToLoad = (MapData)EditorGUILayout.ObjectField("MapData", _mapToLoad, typeof(MapData), false);
        using (new EditorGUILayout.HorizontalScope())
        {
            EditorGUI.BeginDisabledGroup(_mapToLoad == null);
            if (GUILayout.Button("에디터에 적용 (크기·홈·장애물·스포너 칸)", GUILayout.Height(24f)))
                LoadFromMapData(_mapToLoad);
            EditorGUI.EndDisabledGroup();
        }
        EditorGUILayout.HelpBox(
            "프로젝트의 MapData 에셋을 넣고 적용하면, 아래 값과 페인트 상태가 그 맵 기준으로 채워집니다. 저장 시 같은 이름으로 덮어쓰게 됩니다.",
            MessageType.None);
        ...
        _mapBaseName = EditorGUILayout.TextField("맵 이름", _mapBaseName);

        EditorGUILayout.Space(6f);
        if (GUILayout.Button("MapData + 스포너 SO 저장", GUILayout.Height(30f)))
            TrySaveMapAndSpawners();
    ...
    void CreateOrUpdatePreviewGrid()
    {
        ...
        if (root == null)
        {
            ...
            Undo.RegisterCreatedObjectUndo(root, "Create Map Preview");
        }
        ...
        if (plane == null)
        {
            ...
            Undo.RegisterCreatedObjectUndo(p, "Map Preview Plane");
        }
    ...
    void TrySaveMapAndSpawners()
    {
        ...
        string baseName = SanitizeFileName(_mapBaseName);
        if (string.IsNullOrEmpty(baseName))
        {
            EditorUtility.DisplayDialog("저장 불가", "맵 이름이 비었습니다.", "확인");
            return;
        }
        ...
        string mapPath = Path.Combine(MapsFolder, baseName + ".asset").Replace('\\', '/');
        ...
        try
        {
            // MapData를 먼저 에셋으로 등록해야 스포너에서 참조가 유지됨 (미등록 인스턴스 참조는 저장 시 유실될 수 있음).
            MapData map;
            if (File.Exists(mapPath))
            {
                map = AssetDatabase.LoadAssetAtPath<MapData>(mapPath);
                if (map == null)
            ...
            foreach (var cell in sortedSpawnerCells)
            {
                ...
                spso.FindProperty("_mapData").objectReferenceValue = map;
